...
export function BottomNav() {
  ...
  const academySlug = pathname?.split('/a/')?.[1]?.split('/')?.[0] || '';
  
  // Use academy paths if we are in an academy context
  const getPath = (base: string) => {
    if (academySlug) {
      // Mapping legacy/global routes to academy specific ones
      if (base === '/') return `/a/${academySlug}/vocabulario`;
      if (base === '/vocabulario') return `/a/${academySlug}/vocabulario`;
      if (base === '/flashcards') return `/a/${academySlug}/flashcards`;
      if (base === '/quiz') return `/a/${academySlug}/quiz`;
      if (base === '/favoritos') return `/a/${academySlug}/favoritos`;
      if (base === '/progreso') return `/a/${academySlug}/progreso`;
    }
    return base;
  };

  const isHome = pathname === '/' || (academySlug && pathname === `/a/${academySlug}/vocabulario`);
  const isVocab = pathname?.includes('/vocabulario');
  const isFav = pathname?.includes('/favoritos');
  const isFlash = pathname?.includes('/flashcards');
  const isQuiz = pathname?.includes('/quiz');
  const isProgreso = pathname?.includes('/progreso');
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-gray-200 dark:border-slate-800 z-50 pb-safe">
      <div className="max-w-md mx-auto flex justify-around items-center px-2 py-2">
        <Link href={getPath('/')} className={`flex flex-col items-center p-1 rounded-xl transition-all ${isHome ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-blue-500 dark:hover:text-blue-300'}`}>
        ...
        <Link href={getPath('/vocabulario')} className={`flex flex-col items-center p-1 rounded-xl transition-all ${isVocab ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-blue-500 dark:hover:text-blue-300'}`}>
        ...
        <Link href={getPath('/flashcards')} className={`flex flex-col items-center p-1 rounded-xl transition-all ${isFlash ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-blue-500 dark:hover:text-blue-300'}`}>
        ...
        <Link href={getPath('/quiz')} className={`flex flex-col items-center p-1 rounded-xl transition-all ${isQuiz ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-gray-500 hover:text-purple-500 dark:hover:text-purple-300'}`}>
        ...
        <Link href={getPath('/favoritos')} className={`flex flex-col items-center p-1 rounded-xl transition-all ${isFav ? 'text-amber-500 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500 hover:text-amber-500 dark:hover:text-amber-300'}`}>
        ...
        <Link href={getPath('/progreso')} className={`flex flex-col items-center p-1 rounded-xl transition-all ${isProgreso ? 'text-rose-500 dark:text-rose-400' : 'text-gray-400 dark:text-gray-500 hover:text-rose-500 dark:hover:text-rose-300'}`}>
